...
module.exports = class extends Generator {
  ...
  _runGitInitCommand() {
    if (this._projectHasParentFolder('.git')) {
      console.log(
        `Skipping git initialization. .git folder found in parent directory ['.git']`
      );
      return;
    }
  ...
  _useSpecifiedNodeVersion() {
    const nvmIsAvailable =
      process.env.NVM_BIN || process.env.NVM_CD_FLAGS || process.env.NVM_DIR;
    // TO-DO:
    ...
    try {
      const nvmrcNodeVersion = fs
        .readFileSync(`${process.cwd()}/.nvmrc`, 'utf8')
        .trim();
      ...
      console.log(
        chalk.green(
          `Using NodeJS '${nvmrcNodeVersion}' as default NodeJS version`
        )
      );
    } catch (error) {
      console.log(
  ...
  prompting() {
    const prompts = [
      ...
      },
      {
        type: 'input',
        name: 'author',
        message: 'Which team owns this project?',
        store: true,
    ...
    return this.prompt(prompts).then((props) => {
      ...
      newProps.vueAppName = props.vueAppName;
      newProps.vueAppNameSnakeCase = snakeCase(newProps.vueAppName);
      newProps.component = props.vueAppName;
      newProps.componentCC = pascalcase(newProps.component);
      newProps.generatorVersion = generatorPackageJson.version;
      this.props = newProps;
    });
  }
  ...
  writing() {
    ...
    glob
      .sync('./**/*/', { dot: true, cwd: this.sourceRoot() })
      .forEach((dir) => {
